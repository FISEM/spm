...
```
spm add ./blog  →  docker build + docker run  →  http://<IP du VPS>:8001
```

spm ne gère ni domaine, ni TLS, ni reverse proxy : il ouvre des ports, c'est tout.
...
```sh
spm add ./blog                 # premier port libre à partir de 8001
...
Le `.env` du projet est passé au conteneur (`--env-file`) mais jamais copié dans l'image.
